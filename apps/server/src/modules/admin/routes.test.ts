...
describe('chunks CRUD', () => {
  it('GET /api/admin/collections/:id/chunks lists chunks in position order', async () => {
    vi.mocked(session.getSession).mockResolvedValue(adminSession);
    const chunks = [{ id: validId, text: 'sounds good', translation: 'звучит хорошо', explanation: null, example: null, exampleTranslation: null, level: 'A2', situationPrompts: [], hasDialogue: false, position: 0 }];
  ...
  it('POST /api/admin/collections/:id/chunks creates a chunk in the collection', async () => {
    vi.mocked(session.getSession).mockResolvedValue(adminSession);
    const created = { id: validId, text: 'take your time', translation: 'не торопись', explanation: null, example: null, exampleTranslation: null, level: 'A2', situationPrompts: [], hasDialogue: false, position: 3 };
  ...
  it('PATCH /api/admin/chunks/:id updates a chunk', async () => {
    vi.mocked(session.getSession).mockResolvedValue(adminSession);
    const updated = { id: validId, text: 'sounds good', translation: 'звучит здорово', explanation: null, example: null, exampleTranslation: null, level: 'A2', situationPrompts: [], hasDialogue: false };
