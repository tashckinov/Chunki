    ...
    <div className="scroll-clean flex-1 min-h-0 pt-6 pb-8 flex flex-col gap-8">
      {!s.hasProgram && (
        <div className="flex flex-col gap-5 px-5">
          ...
          <Card variant="accent" onClick={() => s.go('goals')} className="p-7 flex flex-col gap-4">
      ...
      {s.hasProgram && currentTopic && (
        <div className="flex flex-col gap-4 px-5">
          <Card variant="accent" onClick={() => s.go('topic')} className="p-7 flex flex-col gap-4">
            <div className="flex items-start justify-between gap-3">
              <div className="text-[12.5px] font-semibold text-accent-strong uppercase tracking-wide">Следующий урок</div>
              <div className="w-11 h-11 flex-none rounded-full bg-accent text-on-accent flex items-center justify-center">
                <Play size={17} fill="currentColor" />
              </div>
            </div>
            <div>
              <div className="text-[24px] font-bold leading-[30px]">{currentTopic.title}</div>
              <div className="text-body-secondary mt-1.5">
                {s.placementResult?.overallLevel ?? s.from} → {s.to} · {nextWhen}
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 pt-1">
              <div className="flex-1 flex gap-1.5">
                {[0, 1, 2, 3].map((i) => (
                  <div key={i} className="flex-1 h-1 rounded-full bg-surface/70" />
                ))}
              </div>
              <div className="text-meta text-accent-strong">{s.minutes} мин</div>
            </div>
          </Card>

          <div className="flex flex-col gap-2 px-1">
            <button onClick={s.toggleCalendar} className="flex items-center gap-2 text-left">
              <div className="flex-1 text-[12px] font-medium text-text-tertiary uppercase tracking-wide">Календарь</div>
              <div className="text-[12.5px] font-medium text-text-tertiary flex items-center gap-0.5">
                {s.calOpen ? 'Свернуть' : 'Все даты'}
                {s.calOpen ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
              </div>
            </button>
            <div className="xscroll-clean -mx-1 px-1 pb-1 flex gap-1.5 overflow-x-auto">
              {calStrip.map((d) => (
                <div key={d.dayIndex} className="flex-none w-8 flex flex-col items-center gap-1">
                  <div className="text-[10px] text-text-tertiary">{d.day}</div>
                  <div
                    className="w-7 h-7 rounded-full flex items-center justify-center text-[12.5px] font-medium"
            ...
            {s.calOpen && (
              <div className="flex flex-col anim-rise pt-2">
                {calList.map((l) => (
                  <ListRow
                    key={l.dayIndex}
                    onClick={l.go}
      ...
      {s.hasProgram && (
        <div className="flex flex-col gap-8">
          ...
          <div className="px-5">
            <Card variant="subtle" onClick={s.goExtras} className="p-5 flex items-center gap-4">
              <div className="w-10 h-10 flex-none rounded-full bg-surface text-accent flex items-center justify-center">
                <PlusCircle size={20} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[15px] font-semibold">Доп. уроки</div>
